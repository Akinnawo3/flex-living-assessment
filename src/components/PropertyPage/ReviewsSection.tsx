...
export default function ReviewsSection({ reviews, propertyId, propertyName }: ReviewsSectionProps) {
  ...
  // Calculate average rating from actual approved reviews
  ...
  return (
    <div className="mt-8 border-t border-gray-200 pt-8">
        ...
        <div>
          ...
          <div className="flex items-center mt-2">
            <div className="flex items-center">
              ...
              <span className="ml-2 text-gray-600">({reviews.length} approved reviews)</span>
            </div>
          </div>
        </div>
      ...
      {reviews.length > 0 ? (
        <>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-blue-50 rounded-xl p-6 mb-8">
          ...
          {filteredReviews.length > 0 ? (
                        ...
                        {review.categories.map((category) => (
                          <div key={category.category} className="flex items-center justify-between">
                            ...
                            <div className="flex items-center">
                              ...
                              <span className="text-sm font-medium w-8 text-gray-400">{category.rating}/10</span>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl">
              ...
              <button onClick={() => setFilterRating(0)} className="mt-2 text-blue-600 hover:text-blue-800">
                Clear filter
              </button>
            </div>
          )}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-12 bg-gray-50 rounded-xl">
          ...
          <p className="text-gray-600 max-w-md mx-auto">This property doesn't have any approved reviews yet. Reviews need to be published in the dashboard before they appear here.</p>
          <a href="/dashboard" className="inline-flex items-center mt-4 text-blue-600 hover:text-blue-800 font-medium">
